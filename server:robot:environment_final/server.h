...
SC_MODULE(Server){
  ...
  sc_in<bool> stop1, stop2, stop3, stop4;//obstacle stops from environment
  ...
  sc_out<bool> go1,go2,go3,go4;//boolean stop and go outs for each robot
  ...
  SC_CTOR(Server){
    ...
    SC_METHOD(robotStopped);
    sensitive<<stop1.pos()<<stop2.pos()<<stop3.pos()<<stop4.pos();
